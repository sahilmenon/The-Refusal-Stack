...
### Changed
- VLM modality leg now renders a FigStep-faithful stimulus (imperative header + blank numbered list, incitement carrier) and adds an OCR-comprehension gate. The gate shows Chameleon refuses even a benign image instruction, so the equal text/image refusal is an instruction-following / OCR confound rather than image-intent recognition. The separate-visual-direction result is unchanged.
