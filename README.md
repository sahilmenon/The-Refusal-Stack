...
- **GCG wasn't doing coordinate descent.** Checking the implementation against Algorithm 1 of the paper revealed the candidate sampler re-drew *every* suffix position from the top-k each step, so each candidate was an almost-random suffix disconnected from the current one — not GCG. Canonical GCG replaces exactly *one* position per candidate (single-token steps), which is what lets the loss make steady progress. Fixed to match the paper; this was the deepest of the GCG bugs and the main suppressor of attack success.
...
Reading the source papers (GCG, PAIR, Arditi, FigStep) against the code surfaced further fidelity gaps beyond the GCG sampler above: Phase 3's refusal-direction *layer selection* uses a separation heuristic, whereas Arditi et al. select the direction whose ablation most reduces refusal on a validation set (the more faithful, causal criterion); PAIR's attacker prompt lacks the paper's role-play framing and in-context examples; and the FigStep VLM stimulus needs the paper's paraphrase-to-imperative + numbered-blank-list rendering. These are scheduled for their respective phases.
