...
Known and deferred (tracked, not yet hit): the PAIR judge routes Llama-Guard's safe/unsafe output through a 1–10 rating parser (needs a rating-capable judge); interp/eval tokenise already-templated strings with `add_special_tokens=True`, a consistent double-BOS to clean up before the Phase-3 run; the Fuyu decoder-module path needs verifying on-pod before the VLM leg.
